...
const StartSession = ({ onBegin, onLogout, loading = false, error = '' }) => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-slate-900 to-violet-900 flex items-center justify-center p-4">
      <div className="w-full max-w-xl bg-white/10 backdrop-blur-2xl border border-white/20 p-10 rounded-[2rem] shadow-2xl">
        <div className="flex items-center justify-between mb-2">
          ...
          <button
            onClick={onLogout}
            className="px-3 py-2 rounded-lg border border-red-500/50 text-red-300 text-xs font-bold hover:bg-red-500/20"
          >
            Logout
          </button>
        </div>
        ...
        <div className="space-y-6">
          <div>
            ...
            >
              ...
              <option className="bg-slate-900">ML Engineer</option>
              <option className="bg-slate-900">GenAI Engineer</option>
              <option className="bg-slate-900">App Developer</option>
              <option className="bg-slate-900">Cybersecurity Engineer</option>
              <option className="bg-slate-900">Blockchain Developer</option>
            </select>
          </div>
          ...
          <button
            ...
            disabled={loading}
            className="w-full bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-black py-5 rounded-2xl shadow-xl shadow-cyan-500/30 transition-all active:scale-95 text-lg uppercase tracking-tighter disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {loading ? 'Launching...' : 'Launch Interview'}
          </button>
        </div>
      </div>
    </div>
